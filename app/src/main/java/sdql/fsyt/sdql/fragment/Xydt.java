 ...
 * 校园动态
 */
public class Xydt extends Fragment {
    private static final String URL = "http://news.jxnu.edu.cn/s/271/t/910/p/17/list.htm";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Cursor c=mSQLiteDatabase.rawQuery("select * from NewsXYDT", null);
        int count=c.getCount();//取得记录总数
        String nowTime=new GetTime().getDay();
        System.out.println("当前日期"+nowTime);
        System.out.println("取得记录总数为："+count);
        c.moveToFirst();
...
//            这种情况就先清空表，避免重复数据
            mSQLiteDatabase.execSQL("delete from NewsXYDT");
            readNet();
        }
        else{
            addData();
    ...
    private void setUI() {
        xwAdapter.setOnItemClickListener(new XwListAdapter.OnRecyclerViewItemClickListener() {
            ...
            public void onItemClick(View view, XwStructure xw) {
                ...
                intent.putExtra("Type","校园动态");//传过去设置标题
    ...
    private void addData() {
        ...
        mDBHelper = new DBHelper(context);
        mSQLiteDatabase = mDBHelper.getReadableDatabase();
        Cursor c=mSQLiteDatabase.rawQuery("select * from NewsXYDT",null);//查询师大要闻表
    ...
    private void insertData(String newsTitle,String newsTime, String newsURL){
        ...
        mSQLiteDatabase.insert("NewsXYDT", null, v);//插入新闻缓存表
    }
    @Override
